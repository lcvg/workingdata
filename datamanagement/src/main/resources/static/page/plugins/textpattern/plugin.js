...
var textpattern = (function () {
  ...
  var $_8fsvf1qjjfeu9lqt = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_8wa5b3qrjfeu9lr8.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_8wa5b3qrjfeu9lr8.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_8wa5b3qrjfeu9lr8.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_bgdw9jqojfeu9lr2.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_bgdw9jqojfeu9lr2.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_bgdw9jqojfeu9lr2.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !global$2.modifierPressed(e)) {
        $_7dt6p2qnjfeu9lqz.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_7dt6p2qnjfeu9lqz.checkKeyCode(keyCodes, e)) {
        $_7dt6p2qnjfeu9lqz.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_7dt6p2qnjfeu9lqz.checkCharCode(charCodes, e)) {
  ...
  var $_57s2h1qkjfeu9lqv = { setup: setup };

  global.add('textpattern', function (editor) {
    var patternsState = Cell($_8fsvf1qjjfeu9lqt.getPatterns(editor.settings));
    $_57s2h1qkjfeu9lqv.setup(editor, patternsState);
    return $_40gkucqijfeu9lqr.get(patternsState);
  });
  function Plugin () {
  }
