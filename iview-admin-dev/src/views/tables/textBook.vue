...
<template>  
    <div>
        ...
        信息提示
        <template slot="desc">1.本页面是出版教材页面，可录入记录、编辑、删除和查看信息。<br>2.开始时间：{{item.beginDate}}，结束时间：{{item.endDate}} </template>
        </Alert>
        <Row class="margin-top-10 margin-bottom-10" >
            ...
            <i-button type="default"  icon="ios-download-outline" class="check" @click="exportTextBook">导出</i-button>
...
export default {
    ...
    methods: {
         exportTextBook(){
            ...
            window.location.href = "/exportTextBook?"+params
        },
        showMod:function() {
            if(this.kj==true){
                this.kjMod = false
        ...
        handleDel(ids){
         ...
         getConfig(){
                let vm = this;
                this.$axios.get('/findConfig',{params:{id:5}})
                    .then(function (response) {
